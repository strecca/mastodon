...
export function MastodonNameLookup(): Plugin {
  ...
  return {
    ...
    generateBundle(options, bundle) {
      if (!root) {
        throw new Error("MastodonNameLookup plugin requires the root to be set in the config.");
      }
      ...
      for (const file in bundle) {
        ...
        const relativePath = relative(root, sanitizeFileName(chunk.facadeModuleId));
        const ext = extname(relativePath);
        const name = chunk.name.replace(ext, "");

        if (nameMap[name]) {
          throw new Error(`Entrypoint ${relativePath} conflicts with ${nameMap[name]}`);
        }
