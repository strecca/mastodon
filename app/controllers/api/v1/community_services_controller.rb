...
class Api::V1::CommunityServicesController < Api::BaseController
  ...
  def index
    ...
    result = cached_list do
      entries = CommunityService.includes(:account)
                                .search(params[:q])
    ...
    inject_list_translations(result, 'CommunityService')
    render json: result
  end
  ...
  def create
    ...
    entry = CommunityService.new(entry_params)
  ...
  def set_entry
    @entry = CommunityService.find(params[:id])
  end
  ...
  def check_rate_limit
    ...
    count = CommunityService.where(account: current_account, created_at: window..).count
