...
class Api::V1::CommunityRestaurantsController < Api::BaseController
  ...
  def index
    ...
    result = cached_list do
      entries = CommunityRestaurant.includes(:account)
                                   .search(params[:q])
  ...
  def create
    ...
    entry = CommunityRestaurant.new(entry_params)
  ...
  def set_entry
    @entry = CommunityRestaurant.find(params[:id])
  end
  ...
  def check_rate_limit
    ...
    count = CommunityRestaurant.where(account: current_account, created_at: window..).count
