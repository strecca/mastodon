...
class Api::V1::CommunityPropertiesController < Api::BaseController
  ...
  def index
    ...
    result = cached_list do
      entries = CommunityProperty.includes(:account)
                                 .search(params[:q])
    ...
    inject_list_translations(result, 'CommunityProperty')
    render json: result
  end
  ...
  def create
    ...
    entry = CommunityProperty.new(entry_params)
  ...
  def set_entry
    @entry = CommunityProperty.find(params[:id])
  end
  ...
  def check_rate_limit
    ...
    count = CommunityProperty.where(account: current_account, created_at: window..).count
