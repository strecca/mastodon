...
const notify = (options) =>
  self.registration.getNotifications().then((notifications) => {
    if (notifications.length >= MAX_NOTIFICATIONS) {
      ...
      group.title = formatMessage("notifications.group", options.data.preferred_locale, {
  ...
  event.waitUntil(
    fetchFromApi(`/api/v1/notifications/${notification_id}`, "get", access_token)
      .then((notification) => {
        ...
        options.title = formatMessage(`notification.${notification.type}`, preferred_locale, {
          name:
