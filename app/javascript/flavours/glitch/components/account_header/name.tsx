...
export const AccountName: FC<{ accountId: string }> = ({ accountId }) => {
  ...
  return (
    <div className={classes.nameWrapper}>
      ...
      <AccountNameHelp username={username} domain={domain} isSelf={account.id === me} />
...
const AccountNameHelp: FC<{
  ...
  return (
    <>
      ...
      >
        {handle}
        <Icon id="help" icon={HelpIcon} aria-label={intl.formatMessage(messages.nameInfo)} />
      </button>

      <Overlay show={open} rootClose target={triggerRef} onHide={handleClick} offset={[5, 5]}>
        {({ props }) => (
          <div
            {...props}
            role="region"
            ...
            <CopyButton value={handle} className={classes.handleCopy}>
              {(wasCopied) => (
                <>
                  ...
                  {wasCopied && <FormattedMessage id="copypaste.copied" defaultMessage="Copied" />}
                </>
              )}
            </CopyButton>
          </div>
