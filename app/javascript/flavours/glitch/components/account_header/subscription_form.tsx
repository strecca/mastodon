...
import type { ValidationErrorResponse, ValidationError } from "@/flavours/glitch/api_types/errors";
...
const isValidationErrorResponse = (data: unknown): data is ValidationErrorResponse =>
  typeof data === "object" && data !== null && "error" in data && "details" in data;
...
export const AccountSubscriptionForm: React.FC<{ accountId: string }> = ({ accountId }) => {
  ...
  if (submitted) {
    return (
      <div className={classNames(classes.bannerBase, classes.bannerBaseCentered)}>
    ...
    <form onSubmit={handleSubmit} className={classes.bannerBase} noValidate>
      ...
      <div className={classes.bannerInputButton}>
        <TextInputField
          ...
          status={errors.email ? fieldStatusFromErrors(intl, errors.email) : undefined}
        />

        <Button type="submit" loading={submitting}>
          <FormattedMessage id="email_subscriptions.form.action" defaultMessage="Subscribe" />
        </Button>
      </div>
