...
const messages = defineMessages({
  ...
  duplicateWarning: {
    id: "community.form.duplicate_warning",
    defaultMessage: "A similar entry already exists in this category — it may be a duplicate:",
  },
...
export const EntryForm = ({ config, mode, entryId, multiColumn }) => {
  ...
  const imagesField = useMemo(
    () => config.fields.find((f) => f.widget === "images"),
    [config.fields],
  );
  ...
  useEffect(() => {
    if (mode === "edit" && currentEntry && !populated) {
      ...
      if (hasImagesField) {
        const ids =
          currentEntry.get("image_media_ids")?.toJS?.() ??
          currentEntry.get("image_media_ids") ??
          [];
        const previews =
          currentEntry.get("image_previews")?.toJS?.() ?? currentEntry.get("image_previews") ?? [];
    ...
    if (mode === "create") {
      ...
      if (textFields.length > 0) {
        ...
        textFields.forEach((f) => {
          checkParams[f.db_name] = formData[f.db_name] || "";
        });
        checkingDuplicate.current = true;
        try {
          const res = await api().get("/api/v1/community_directory/duplicate_check", {
            params: checkParams,
          });
  ...
  const groupOrder =
    config.groups?.length > 0 ? config.groups.map((g) => g.name) : Object.keys(groupedFields);
  ...
  return (
    <div className="scrollable community-entry-form">
      {success && (
        <div className="community-entry-form__success">{intl.formatMessage(messages.success)}</div>
      )}

      {/* Image upload — shown once above the field groups if config has images widget */}
      {hasImagesField && (
        <div className="community-entry-form__group">
          <legend className="community-entry-form__group-title">
            {intl.formatMessage(messages.photosHeading, { max: MAX_CATEGORY_IMAGES })}
          </legend>
          <div className="community-entry-form__image-row">
            {images.map((img, idx) => (
              <div key={img.mediaId} className="community-entry-form__image-thumb">
                ...
                <button
                  type="button"
                  className="community-entry-form__image-remove"
            ...
            {images.length < MAX_CATEGORY_IMAGES && (
              ...
              >
                {uploading
                  ? uploadStatus || intl.formatMessage(messages.uploading)
      ...
      {groupOrder.map((groupName) => {
        ...
        const label =
          groupLabels[groupName] || (groupName !== "default" ? humanize(groupName) : null);
        const cols = groupColumns[groupName] || 1;

        return (
          <fieldset key={groupName} className="community-entry-form__group">
            {label && <legend className="community-entry-form__group-title">{label}</legend>}
      ...
      {duplicateWarning && (
        <div className="community-entry-form__duplicate-warning">
          ...
          <ul className="community-entry-form__duplicate-list">
            {duplicateWarning.map((match) => (
              <li key={match.id} className="community-entry-form__duplicate-item">
                ...
                <span className={`cd-status-badge cd-status-badge--${match.status}`}>
                  {match.status}
                </span>
                <Link
          ...
          <div className="community-entry-form__duplicate-actions">
            ...
            <button type="button" className="button" onClick={handleSubmitAnyway}>
...
const FormField = ({ field, value, error, onChange, intl }) => {
  ...
  const colClass =
    field.column === "1"
      ? "community-entry-form__field--col1"
      : field.column === "2"
        ? "community-entry-form__field--col2"
        : "community-entry-form__field--full";

  const handleInput = useCallback(
    (e) => onChange(field.db_name, e.target.value),
    [onChange, field.db_name],
  );
  ...
  switch (field.widget) {
    ...
    case "select":
      input = (
        ...
        >
          <option value="">
            {intl.formatMessage({ id: "community.form.select", defaultMessage: "Select…" })}
          </option>
          {(field.options || []).map((opt) => (
            <option key={opt} value={opt}>
              {optionLabel(field, opt, locale)}
            </option>
          ))}
        </select>
      );
    ...
    case "location_select":
      ...
      return (
        <div className={`community-entry-form__field ${colClass}`}>
          ...
          {input}
          {error && (
            <p id={errorId} className="community-entry-form__error">
              {error}
            </p>
          )}
        </div>
    ...
    case "checkboxes":
      ...
      return <div className={`community-entry-form__field ${colClass}`}>{input}</div>;
