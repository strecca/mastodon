...
const messages = defineMessages({
  ...
  deleteConfirm: {
    id: "community.detail.delete_confirm",
    defaultMessage: "Permanently delete this entry? This cannot be undone.",
  },
...
const EntryDetailInner = ({ config, entryId, identity }) => {
  ...
  const isOwner =
    signedIn && account && (identity?.accountId === String(account.get("id")) || isAdmin);
  ...
  const nameField =
    !hasNameParts &&
    config.fields.find((f) => ["display_name", "name", "title", "event_name"].includes(f.db_name));
  ...
  const badgeFields = config.fields.filter(
    (f) => f.show_in_detail !== false && ["checkboxes", "select", "radio"].includes(f.widget),
  );
  const badges = [];
  ...
  const locationField = config.fields.find(
    (f) =>
  ...
  const visibleFields = config.fields.filter(
    (f) => f.show_in_detail !== false && !heroDbNames.has(f.db_name) && f.widget !== "images",
  );
  const groupMap = {};
  ...
  const groupOrder =
    config.groups?.length > 0 ? config.groups.map((g) => g.name) : Object.keys(groupMap);
          ...
          <div className="community-entry-detail__hero-content">
            {badges.length > 0 && (
              <div className="community-entry-detail__badges">
                {badges.map((v, i) => (
                  <span key={i} className="community-entry-detail__badge">
                    {v}
                  </span>
                ))}
              </div>
        ...
        <div className="community-entry-detail__card">
          ...
          {account && (
            <div className="community-entry-detail__account">
              ...
              <div className="community-entry-detail__timestamps">
                {createdAt && (
                  <span>
                    {intl.formatMessage(messages.added)} <RelativeTimestamp timestamp={createdAt} />
                  </span>
                )}
              ...
              <div key={groupName} className="community-entry-detail__group">
                ...
                <dl className="community-entry-detail__fields">
                  {groupFields.map((field) => {
                    ...
                    const storedVal =
                      rawVal && typeof rawVal.toJS === "function"
                        ? rawVal
                            .toJS()
                            .map((v) => (isOptionField ? optionLabel(field, v, locale) : v))
                            .join(", ")
                        : isOptionField
                          ? optionLabel(field, String(rawVal), locale)
                          : String(rawVal);
                    const rawDisplayVal = field.translatable
                      ? translatedValue(entry, field.db_name, locale) || storedVal
                      : storedVal;
                    const val =
                      field.widget === "date" ? fmtFieldDate(rawDisplayVal, locale) : rawDisplayVal;
                    ...
                    return (
                      <div key={field.db_name} className="community-entry-detail__field">
                        ...
                        <dd
                          className={`community-entry-detail__field-value${field.widget === "textarea" ? " community-entry-detail__field-value--multiline" : ""}`}
                        >
                          {isLink ? (
                            <a
                              href={href}
                              ...
                              className="community-entry-detail__link"
                            >
                              {val}
                            </a>
                          ) : (
          ...
          {isOwner && (
            <div className="community-entry-detail__actions">
              ...
              {isAdmin && (
                <button type="button" className="button button--destructive" onClick={handleDelete}>
