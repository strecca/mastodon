    ...
    <Column>
      ...
      <div className="newsletter-index scrollable">
        <div className="newsletter-index__masthead">
          ...
          <p className="newsletter-index__subtitle">La voce scritta della comunita di Civezza</p>
          <div className="newsletter-index__rule" />
        </div>

        {loading && <div className="newsletter-index__loading">Caricamento...</div>}

        {!loading && newsletters.length === 0 && (
          <div className="newsletter-index__empty">Nessuna newsletter pubblicata.</div>
        )}

        <div className="newsletter-index__grid">
          {newsletters.map((nl) => (
            <Link key={nl.slug} to={`/newsletters/${nl.slug}`} className="newsletter-card">
              <div className="newsletter-card__meta">
                <span className="newsletter-card__location">
                  {nl.masthead_location || "Civezza"}
                </span>
              ...
              {nl.excerpt_it && <p className="newsletter-card__excerpt">{nl.excerpt_it}</p>}
            </Link>
          ))}
        </div>
      </div>
