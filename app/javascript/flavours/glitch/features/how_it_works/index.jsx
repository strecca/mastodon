...
const SECTION_DEFAULTS = {
  join: {
    title: "Joining & Signing In",
    body: 'Registration is free — just click "Join" or "Create Account" from the home page.',
  },
  community: {
    title: "The Community Hub",
    body: 'Click "Community" in the menu to see the full directory as a grid of tiles.',
  },
  listings: {
    title: "Browsing Listings & Categories",
    body: "Buy, sell, trade, or give away items, and browse Services, Restaurants, Properties, and Artists.",
  },
  visits: {
    title: "When I'm In Town",
    body: "Privately share your visit dates so friends can plan to meet up.",
  },
  events: {
    title: "Community Events",
    body: "A live calendar of concerts, festivals, markets, and gatherings.",
  },
  digest: {
    title: "The Daily Digest",
    body: "A bilingual daily newspaper of local events and community news.",
  },
  newsletters: {
    title: "Newsletters",
    body: "Longer community stories and updates, published from time to time.",
  },
  stories: {
    title: "Member Stories",
    body: "Residents and visitors share their own connection to Civezza.",
  },
  language: {
    title: "Switching Languages",
    body: "Tap the globe icon at the top of the menu to change viewing language.",
  },
  contact: {
    title: "Getting Help",
    body: 'Use "Contact Admin" in the menu to send a message directly to the team.',
  },
};
...
const HowItWorks = ({ identity }) => {
  ...
  const appIconUrl =
    server?.item?.icon?.find((i) => i.size === "192x192")?.src ?? server?.item?.icon?.at(-1)?.src;
    ...
    <div className="hiw-page scrollable">
      <Helmet>
        ...
        <meta
          ...
          content={sc(
            "guide_page_intro",
            "A quick tour of everything you can do on MiaCivezza.com.",
          )}
        />
      </Helmet>
      ...
      <header className="hiw-page__hero">
        ...
        <p className="hiw-page__intro">
          {sc(
            "guide_page_intro",
            "MiaCivezza.com is more than a social network — it's a digital piazza for the Civezza and Imperia coast community. Here's a quick tour of everything you can do.",
          )}
        </p>
      ...
      <div className="hiw-page__bio">
        ...
        <div className="hiw-page__bio-text">
          ...
          <p className="hiw-page__bio-body">
            {sc(
              "about_bio_p1",
              'MiaCivezza.com is my attempt to provide an online Community Bulletin Board and Piazzetta meeting plaza. My Wife & I would meet people at dinner or aperitivo time or at artistic or musical events, make friends and then say Goodbye and want to stay in contact with them. We\'d try to exchange contact information as the last process of saying "We hope to see you again".',
            )}
          </p>
          <p className="hiw-page__bio-body">
            {sc(
              "about_bio_p2",
              "Now Visitors and Residents have immediate access to contact each other to renew friendships and to reunite.",
            )}
          </p>
        </div>
      </div>
      ...
      <div className="hiw-page__intro-card">
        <h2 className="hiw-page__intro-title">
          {sc("guide_intro_title", "What is MiaCivezza.com?")}
        </h2>
        <ul className="hiw-page__intro-body">
          {toBullets(sc("guide_intro_body", INTRO_BODY_FALLBACK)).map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
      </div>
      ...
      <div className="hiw-page__tip">
        <h2 className="hiw-page__tip-title">
          {sc("guide_tip_title", "💡 Tip: Start on a computer, if you can")}
        </h2>
        <ul className="hiw-page__tip-body">
          {toBullets(sc("guide_tip_body", TIP_BODY_FALLBACK)).map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
      </div>
      ...
      <div className="hiw-page__install">
        ...
        <div className="hiw-page__install-illustrations">
          <div className="hiw-phone-mock">
            <div className="hiw-phone-mock__screen">
              ...
              <div className="hiw-phone-mock__app hiw-phone-mock__app--real">
                {appIconUrl && <img src={appIconUrl} alt="" className="hiw-phone-mock__app-icon" />}
          ...
          <div className="hiw-tap-illustration">
            <div className="hiw-tap-illustration__icon-wrap">
              {appIconUrl && <img src={appIconUrl} alt="" className="hiw-tap-illustration__icon" />}
        ...
        <p className="hiw-page__install-caption">
          {sc(
            "guide_install_tap_caption",
            "One tap, straight in — no browser tabs to dig through.",
          )}
        </p>
      </div>
        ...
        {SECTIONS.map(({ key, Icon, to, bg }) => (
          <section key={key} className="hiw-page__section" style={{ "--section-bg": bg }}>
            ...
            <div className="hiw-page__section-body">
              <h3 className="hiw-page__section-title">
                {sc(`guide_${key}_title`, SECTION_DEFAULTS[key].title)}
              </h3>
              <p className="hiw-page__section-text">
                {sc(`guide_${key}_body`, SECTION_DEFAULTS[key].body)}
              </p>
              {to && (
