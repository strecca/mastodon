...
const getFilterRange = (key) => {
  ...
  switch (key) {
    ...
    case "next_week": {
      const ws = new Date(t);
      ws.setDate(t.getDate() - t.getDay() + 7);
      const we = new Date(ws);
      we.setDate(ws.getDate() + 6);
      return {
        from: isoDate(ws),
        to: isoDate(we),
        navDate: { year: ws.getFullYear(), month: ws.getMonth() },
      };
    }
    default:
      return null;
...
const fmtDateRange = (a, b) => {
  ...
  if (a === b)
    return da.toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" });
    ...
    <div className="cv-teaser">
      <div className="cv-teaser__hero">
        ...
        <p>
          {sc(
            "visits_teaser_body",
            "Members share their visit dates so connections can meet up. Sign in to see who's coming and add your own dates.",
          )}
        </p>
      </div>
          ...
          <span
            ...
            style={{
              "--ring": v.get("is_own")
                ...
                : (RING_COLORS[rel] ?? RING_COLORS.none),
            }}
      ...
      {isExpanded && (
        <div className="cv-list__detail">
          <div className="cv-list__detail-inner">
            <div className="cv-list__detail-dates">{fmtDateRange(arrival, departure)}</div>
            ...
            {isOwn && !isPast && (
              <div className="cv-list__detail-actions">
                ...
                <button type="button" className="cv-list__action-btn" onClick={() => onEdit(visit)}>
                  <EditIcon /> Edit
                </button>
                <button
                  type="button"
...
const VisitDateGroup = ({ groupKey, visits, expandedId, onToggle, onEdit, onDelete, onNotify }) => {
  ...
  const isTomorrow = (() => {
    ...
    return groupKey === isoDate(t);
  })();
  const label = isCurrent
    ? "Currently In Town"
    : isTomorrow
      ? `Tomorrow — ${fmtDate(groupKey)}`
      : fmtDate(groupKey);

  return (
    <div className="cv-list__group">
      <div
        className={`cv-list__date-heading${isCurrent ? " cv-list__date-heading--current" : isTomorrow ? " cv-list__date-heading--tomorrow" : ""}`}
      >
        {label}
        <span className="cv-list__date-count">
    ...
    <div className={`cv-my-card${isPast ? " cv-my-card--past" : ""}`}>
      ...
      {avail.length > 0 && (
        <div className="cv-my-card__chips">
          {avail.map((k) => (
            <span key={k} className="cv-my-card__chip">
              {AVAILABILITY_LABELS[k] || k}
            </span>
          ))}
        </div>
      )}
      {visit.get("note") && <p className="cv-my-card__note">{visit.get("note")}</p>}
...
const CommunityVisits = ({ multiColumn }) => {
  ...
  const filteredVisits = useMemo(() => {
    ...
    if (q) {
      result = result.filter((v) => {
        const acct = v.get("account");
        return (
          acct?.get("display_name")?.toLowerCase().includes(q) ||
    ...
    if (activeFilter) {
      ...
      if (range) {
        result = result.filter(
          (v) => v.get("arrival_date") <= range.to && v.get("departure_date") >= range.from,
        );
      }
    }
  ...
  const listHeader = useMemo(() => {
    ...
    const inTownNow = listVisits.some(
      (v) =>
    ...
    const thisWeek = listVisits.some(
      (v) =>
        (v.get("arrival_date") ?? "") <= weEndIso && (v.get("departure_date") ?? "") >= todayIso,
    );
        ...
        <>
          ...
          <div className="cv-filters">
            ...
            {hasActiveFilters && (
              <button
                type="button"
                className="cv-filter-chip cv-filter-chip--clear"
          ...
          <div className="cv-date-range">
            <label className="cv-date-range__label">
              ...
              <input
                type="date"
                className="cv-date-range__input"
                value={dateFrom}
            ...
            <>
              ...
              <div className="cv-cal">
                {WEEKDAY_LABELS.map((d) => (
                  <div key={d} className="cv-cal__weekday">
                    {d}
                  </div>
                ))}
                        ...
                        <div
                          ...
                          onKeyDown={
                            clickable
                              ? (e) =>
                                  (e.key === "Enter" || e.key === " ") &&
                                  openDayPanel(key, dayVisits)
                              : undefined
                          }
                          aria-label={
                            clickable
                              ? `${day.getDate()} — ${dayVisits.length} visitor${dayVisits.length === 1 ? "" : "s"}`
                              : undefined
                          }
                        >
            ...
            <div className="cv-my-section">
              ...
              {sortedMyVisits.size === 0 ? (
                <p className="cv-my-section__empty">
                  You haven&#39;t added any visits yet. Let connections know when you&#39;ll be in
                  town!
                </p>
              ) : (
