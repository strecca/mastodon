...
export const NotifyFriendsModal = ({ visit, onClose }) => {
  ...
  const toggleAccount = (id) => setSelected((prev) => ({ ...prev, [id]: !prev[id] }));
      ...
      >
        ...
        <div className="cv-modal__body">
          <p className="cv-notify__subtitle">
            ...
            <strong>
              {fmtDate(visit.get("arrival_date"))}–{fmtDate(visit.get("departure_date"))}
            </strong>
            .
          </p>
            ...
            <>
              {candidates.length === 0 ? (
                ...
                <ul className="cv-notify__list">
                  {candidates.map((c) => {
                    ...
                    const ringColor =
                      rel === "my_people" ? "#7b1fa2" : (RING_COLORS[rel] ?? RING_COLORS.none);
                    return (
                      <li key={acct.id} className="cv-notify__item">
                        <label className="cv-notify__label">
                          ...
                          />
                          <span className="cv-notify__avatar-wrap" style={{ "--ring": ringColor }}>
                            <img
              ...
              <div className="cv-form__actions">
                ...
                >
                  {sending
                    ? "Sending…"
                    : `Notify ${selectedIds.length} ${selectedIds.length === 1 ? "person" : "people"}`}
                </button>
              </div>
            </>
          )}
