...
const buildVisibilityOptions = (myPeopleCount) => [
  {
    ...
    label: "All members",
    desc: "Everyone who has joined can see you're in town",
  },
  {
    value: "connections_only",
    label: "My connections",
    desc: "Only people you follow or who follow you",
  },
  {
    value: "my_people",
    label: "My People",
    desc:
      myPeopleCount > 0
        ? `Your hand-picked group of ${myPeopleCount} — manage below`
        : "Your hand-picked group — add members below",
  },
  {
    value: "ghost",
    label: "Private",
    desc: "Only you can see this — useful for planning your own dates",
  },
];
...
export const VisitForm = ({ visit, onClose, onSaved, myPeopleCount = 0 }) => {
  ...
  const [visibility, setVisibility] = useState(
    visit ? visit.get("visibility") : "public_to_members",
  );
    ...
    <div className="cv-modal-backdrop" onClick={onClose}>
      ...
      >
        <div className="cv-modal__header">
          <h2 className="cv-modal__title">{visit ? "Edit your visit" : "Add a visit"}</h2>
        ...
        <form className="cv-modal__body" onSubmit={handleSubmit}>
          ...
          <div className="cv-form__date-row">
            <div className="cv-form__field">
              <label className="cv-form__label" htmlFor="cv-arrival">
                Arriving
              </label>
              <input
                id="cv-arrival"
            ...
            <div className="cv-form__field">
              <label className="cv-form__label" htmlFor="cv-departure">
                Leaving
              </label>
              <input
                id="cv-departure"
          ...
          <fieldset className="cv-form__fieldset">
            ...
            <div className="cv-form__radio-group">
              {visibilityOptions.map((opt) => (
                <label
                  key={opt.value}
                  className={`cv-form__radio-card${visibility === opt.value ? " cv-form__radio-card--active" : ""}`}
                >
                  <input
                    type="radio"
                    name="visibility"
            ...
            <fieldset className="cv-form__fieldset">
              ...
              <div className="cv-form__check-grid">
                {AVAILABILITY_OPTIONS.map((opt) => (
                  <label
                    key={opt.value}
                    className={`cv-form__check-pill${availabilities.includes(opt.value) ? " cv-form__check-pill--on" : ""}`}
                  >
                    <input
                      type="checkbox"
          ...
          {
            <div className="cv-form__field">
              <label className="cv-form__label" htmlFor="cv-note">
                Note <span className="cv-form__optional">(optional)</span>
              </label>
              <textarea
                id="cv-note"
