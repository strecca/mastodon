...
const FollowerCount: React.FC<{ accountId: string }> = ({ accountId }) => {
  ...
  return <ShortNumber value={account.followers_count} renderer={FollowersCounter} />;
};
...
export const NotificationFollow: React.FC<{
  ...
  const username = useAppSelector((state) => state.accounts.getIn([me, "username"]) as string);
  ...
  if (notification.sampleAccountIds.length === 1) {
    ...
    if (account) {
      actions = <FollowButton compact accountId={notification.sampleAccountIds[0]} />;
