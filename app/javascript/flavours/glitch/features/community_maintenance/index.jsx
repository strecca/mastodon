...
const ConfirmDelete = ({ label, count, onConfirm, busy }) => {
  ...
  return (
    <div className="cm-confirm-row">
      <span className="cm-confirm-label">
        Delete {count} {label}?
      </span>
      <button
...
const Toast = ({ message, onDismiss }) => (
  <div className="cm-toast" onClick={onDismiss}>
    {message} ✕
  </div>
);
...
const CommunityMaintenance = ({ multiColumn }) => {
  ...
  const act = useCallback(
    async (method, path, params = {}, label) => {
      setBusy(true);
      try {
  ...
  return (
    <Column bindToDocument={!multiColumn} label="Site Maintenance">
      ...
      />
      <Helmet>
        <title>Site Maintenance · miacivezza</title>
      </Helmet>
      ...
      <div className="cm-page">
        ...
        <section className="cm-section cm-section--quick">
          ...
          <div className="cm-quick-row">
            <button
              ...
              onClick={() => post("run_all_scrapers", {}, "All scrapers")}
              disabled={busy}
            >
        ...
        <section className="cm-section">
          ...
          <div className="cm-stat-row">
            <span className="cm-stat">
              <strong>{s.events.upcoming}</strong> upcoming
            </span>
            <span className="cm-stat cm-stat--warn">
              <strong>{s.events.past}</strong> past
            </span>
            <span className="cm-stat">
              <strong>{s.events.pending}</strong> pending approval
            </span>
          </div>
        ...
        <section className="cm-section">
          ...
          <div className="cm-stat-row">
            <span className="cm-stat">
              <strong>{s.listings.open}</strong> open
            </span>
            <span className="cm-stat">
              <strong>{s.listings.fulfilled}</strong> fulfilled
            </span>
            <span className="cm-stat">
              <strong>{s.listings.closed}</strong> closed
            </span>
            <span className="cm-stat cm-stat--warn">
              <strong>{s.listings.stale}</strong> stale (&gt;30 days)
            </span>
          </div>
          <div className="cm-action-row">
            ...
            <ConfirmDelete
              ...
              busy={busy}
              onConfirm={() =>
                act("delete", "stale_listings", { days: listingDays }, "Stale listings deleted")
              }
            />
          </div>
        </section>
        ...
        <section className="cm-section">
          ...
          <div className="cm-stat-row">
            <span className="cm-stat">
              <strong>{s.visits.active}</strong> in town now
            </span>
            <span className="cm-stat">
              <strong>{s.visits.upcoming}</strong> upcoming
            </span>
            <span className="cm-stat cm-stat--warn">
              <strong>{s.visits.past}</strong> past
            </span>
          </div>
          <div className="cm-action-row">
            <ConfirmDelete
        ...
        <section className="cm-section">
          ...
          <div className="cm-scraper-grid">
            {SCRAPERS.map((sc) => {
              const log =
                s.scrapers.find((r) => r.source.includes(sc.label.split(" ")[0])) ||
                s.scrapers.find((r) => r.source === sc.key);
              const failed = log?.status === "failed";
              return (
                <div
                  key={sc.key}
                  className={`cm-scraper-card${failed ? " cm-scraper-card--failed" : ""}`}
                >
                  <div className="cm-scraper-card__name">{sc.label}</div>
                  {log ? (
                    <>
                      <div
                        className={`cm-scraper-card__status cm-scraper-card__status--${log.status}`}
                      >
                        {log.status}
                      </div>
                      <div className="cm-scraper-card__meta">Last run: {fmtDate(log.ran_at)}</div>
                      <div className="cm-scraper-card__meta">
                        {log.fetched} fetched · {log.imported} imported · {log.skipped} skipped
                      </div>
          ...
          <div className="cm-action-row cm-action-row--mt">
            ...
            <ConfirmDelete
              ...
              busy={busy}
              onConfirm={() =>
                act("delete", "scraper_logs", { days: logDays }, "Scraper logs cleared")
              }
            />
        ...
        <section className="cm-section">
          ...
          <div className="cm-stat-row">
            <span className="cm-stat">
              <strong>{s.storage.protected_ids}</strong> media files protected from vacuum
            </span>
            <span className="cm-stat cm-stat--warn">
              <strong>{s.storage.orphaned}</strong> orphaned attachments (will be cleaned next
              vacuum)
            </span>
          </div>
          <div className="cm-action-row">
