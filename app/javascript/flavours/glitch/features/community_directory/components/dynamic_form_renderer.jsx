...
const CommunityDirectoryAdmin = () => {
  ...
  const saveSchema = () => {
    ...
    alert(
      `Schema for "${categoryName}" saved with ${fields.length} fields.\n\nYAML file would be created here.`,
    );
    ...
    <div className="status">
      ...
      {step === 2 && (
        <div>
          ...
          <p className="lead">
            Define the fields that will appear in forms and the database table.
          </p>
          ...
          <div className="status mb-4">
            ...
            <div className="d-flex gap-3 flex-wrap">
              <input
                placeholder="db_name (e.g. full_name)"
                value={newField.db_name}
                onChange={(e) => setNewField({ ...newField, db_name: e.target.value })}
              ...
              <select
                ...
                onChange={(e) => setNewField({ ...newField, widget: e.target.value })}
                className="search__input"
              >
          ...
          {fields.length > 0 ? (
            fields.map((field, index) => (
              <div
                key={index}
                className="status d-flex justify-content-between align-items-center mb-2"
              >
                <div>
                ...
                <button onClick={() => removeField(index)} className="btn btn-danger btn-sm">
                  Remove
                </button>
              </div>
            ))
      ...
      {status && (
        <div className={`status mt-4 ${status.includes("✅") ? "success" : "error"}`}>{status}</div>
      )}
    </div>
  );
};
