...
const messages = defineMessages({
  ...
  category: {
    id: "community_directory.permissions.category",
    defaultMessage: "Category (leave blank for global)",
  },
  trusted: {
    id: "community_directory.permissions.trusted",
    defaultMessage: "Trusted — auto-approve entries",
  },
  steward: {
    id: "community_directory.permissions.steward",
    defaultMessage: "Steward — can approve others' entries",
  },
  ...
  empty: {
    id: "community_directory.permissions.empty",
    defaultMessage: "No special permissions set.",
  },
  allCategories: { id: "community_directory.permissions.all", defaultMessage: "All categories" },
  addHeading: {
    id: "community_directory.permissions.add",
    defaultMessage: "Add or update permission",
  },
  listHeading: {
    id: "community_directory.permissions.list",
    defaultMessage: "Existing permissions",
  },
});
...
const CommunityDirectoryPermissions = ({ multiColumn }) => {
  ...
  const loading = useAppSelector(
    (s) => s.get("community_directory")?.get("permissionsLoading") || false,
  );
      ...
      <div className="scrollable">
        <div className="cd-permissions">
          ...
          <section className="cd-permissions__section">
            <h3 className="cd-permissions__heading">{intl.formatMessage(messages.addHeading)}</h3>
          ...
          <section className="cd-permissions__section">
            <h3 className="cd-permissions__heading">{intl.formatMessage(messages.listHeading)}</h3>

            {loading ? (
              <LoadingIndicator />
            ) : !permissions || permissions.size === 0 ? (
              <p className="cd-permissions__empty">{intl.formatMessage(messages.empty)}</p>
            ) : (
                ...
                {permissions.map((perm) => (
                  <div key={perm.get("id")} className="cd-permissions__item">
                    ...
                    <div className="cd-permissions__badges">
                      {perm.get("trusted") && (
                        <span className="cd-permissions__badge cd-permissions__badge--trusted">
                          Trusted
                        </span>
                      )}
                      {perm.get("is_steward") && (
                        <span className="cd-permissions__badge cd-permissions__badge--steward">
                          Steward
                        </span>
                      )}
                    </div>
