...
const messages = defineMessages({
  ...
  addField: {
    id: "community_directory.edit.add_field",
    defaultMessage: "Add new field (adds DB column)",
  },
  addGroup: { id: "community_directory.edit.add_group", defaultMessage: "Add group" },
  existingFields: {
    id: "community_directory.edit.existing_fields",
    defaultMessage: "Existing Fields",
  },
  newFields: { id: "community_directory.edit.new_fields", defaultMessage: "New Fields" },
  newFieldsHint: {
    id: "community_directory.edit.new_fields_hint",
    defaultMessage: "Each new field adds a column to the database.",
  },
...
const CommunityDirectoryEdit = ({ multiColumn }) => {
  ...
  const handleAddNewField = useCallback(
    () => setNewFields((prev) => [...prev, { ...EMPTY_NEW_FIELD }]),
    [],
  );
  const handleNewFieldChange = useCallback(
    (index, updated) => setNewFields((prev) => prev.map((f, i) => (i === index ? updated : f))),
    [],
  );
  const handleRemoveNewField = useCallback(
    (index) => setNewFields((prev) => prev.filter((_, i) => i !== index)),
    [],
  );
  ...
  const handleAddGroup = useCallback(
    () => setGroups((prev) => [...prev, { name: "", label: "", columns: 1 }]),
    [],
  );
  ...
  const handleRemoveGroup = useCallback(
    (index) => setGroups((prev) => prev.filter((_, i) => i !== index)),
    [],
  );
  ...
  if (loadError) {
    return (
      <Column bindToDocument={!multiColumn} ref={column} label={title}>
        <ColumnHeader icon="cog" title={title} multiColumn={multiColumn} showBackButton />
        <div className="scrollable">
          <div className="empty-column-indicator">{loadError}</div>
        </div>
      </Column>
    );
  }
        ...
        <div className="cd-builder">
          ...
          {result && (
            <div className="cd-builder__success">
              ...
              {result.new_columns?.length > 0 && (
                <p>New DB columns: {result.new_columns.join(", ")}</p>
              )}
              {result.files_created?.length > 0 && (
                <p>Files created: {result.files_created.length}</p>
              )}
            </div>
          )}
          ...
          <section className="cd-builder__section">
            <h3 className="cd-builder__section-title">
              {intl.formatMessage(messages.metaSection)}
            </h3>
...
const ExistingFieldRow = ({
  ...
  <div className="cd-field-editor">
    <div className="cd-field-editor__header">
      ...
      <span className="cd-field-editor__locked-type">
      ...
      <div className="cd-field-editor__move">
        <button
          ...
          disabled={isFirst}
          className="cd-field-editor__move-btn"
          title="Move up"
        >
          ↑
        </button>
    ...
    <div className="cd-field-editor__body">
      ...
      <div className="cd-field-editor__toggles">
        ...
        <label className="cd-field-editor__toggle">
          <input
            type="checkbox"
            checked={field.show_in_detail !== false}
