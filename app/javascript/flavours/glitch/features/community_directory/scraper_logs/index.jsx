...
const LatestCard = ({ log }) => (
  <div className={`scraper-logs__source-card scraper-logs__source-card--${log.status}`}>
    ...
    <div className="scraper-logs__source-stats">
      <span className="scraper-logs__stat">
        <strong>{log.fetched}</strong> fetched
      </span>
      <span className="scraper-logs__stat scraper-logs__stat--imported">
        <strong>{log.imported}</strong> imported
      </span>
      <span className="scraper-logs__stat">
        <strong>{log.skipped}</strong> skipped
      </span>
    </div>
    <StatusBadge status={log.status} />
    {log.error_message && <div className="scraper-logs__error-msg">{log.error_message}</div>}
  </div>
);
...
const LogRow = ({ log }) => (
  <div className="scraper-logs__row">
    ...
    <span className="scraper-logs__row-time" title={fmtDatetime(log.ran_at)}>
      {timeAgo(log.ran_at)}
    </span>
    ...
    <StatusBadge status={log.status} />
    {log.error_message && (
      <span className="scraper-logs__row-error" title={log.error_message}>
        ⚠ error
      </span>
    )}
  </div>
...
export default function CommunityDirectoryScraperLogs({ multiColumn }) {
  ...
  return (
    <Column>
      ...
      <Helmet>
        <title>Scraper Logs · miacivezza</title>
      </Helmet>

      <div className="scraper-logs">
        <div className="scraper-logs__nav">
          <Link to="/community_directory" className="scraper-logs__back">
            ← Community Admin
          </Link>
        </div>
        ...
        {data && (
          <>
            {data.latest.length === 0 ? (
              ...
              <div className="scraper-logs__sources">
                {data.latest.map((l) => (
                  <LatestCard key={l.source_name} log={l} />
                ))}
              </div>
            )}
