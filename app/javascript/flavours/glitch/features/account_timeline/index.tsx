...
import { expandTimelineByKey, timelineKey } from "@/flavours/glitch/actions/timelines_typed";
...
const AccountTimeline: FC<{ multiColumn: boolean }> = ({ multiColumn }) => {
  ...
  return (
    <AccountTimelineContext.Provider value={accountContext}>
      <InnerTimeline accountId={accountId} key={accountId} multiColumn={multiColumn} />
...
const InnerTimeline: FC<{ accountId: string; multiColumn: boolean }> = ({
  ...
  const { isLoading: isPinnedLoading, statusIds: pinnedStatusIds } = usePinnedStatusIds({
    accountId,
    tagged,
    forceEmptyState,
  });
...
const EmptyMessage: FC<{ accountId: string }> = ({ accountId }) => {
  ...
  return <FormattedMessage id="empty_column.account_timeline" defaultMessage="No posts found" />;
};
