...
const fmtPrice = (listing) => {
  ...
  const amt = parseFloat(listing.price).toLocaleString("it-IT", {
    style: "currency",
...
const InterestQueue = ({ interests, listingId, onRefresh }) => {
  ...
  if (!interests?.length)
    return <div className="cl-interests__empty">No one has expressed interest yet.</div>;
      ...
      {interests.map((i) => (
        <div key={i.id} className={`cl-interests__row cl-interests__row--${i.status}`}>
          ...
          <div className="cl-interests__actions">
                ...
                <button
                  ...
                  onClick={() =>
                    handle(
                      () =>
                        api().put(
                          `/api/v1/community_listings/${listingId}/interests/${i.id}/select`,
                        ),
                      i.id,
                    )
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    handle(
                      () =>
                        api().put(
                          `/api/v1/community_listings/${listingId}/interests/${i.id}/dismiss`,
                        ),
                      i.id,
                    )
                  }
            ...
            {i.status === "selected" && (
              <span className="cl-interests__status-badge cl-interests__status-badge--selected">
                Selected
              </span>
            )}
            {i.status === "dismissed" && (
              <span className="cl-interests__status-badge cl-interests__status-badge--dismissed">
                Dismissed
              </span>
            )}
          </div>
...
const CommunityListingsShow = ({ multiColumn, params }) => {
  ...
  if (!listing)
    return (
      <Column>
        <div style={{ padding: 20 }}>Listing not found.</div>
      </Column>
    );
  ...
  const statusLabel =
    listing.status === "open"
      ? null
      : listingOptionLabel(STATUS_LABELS, listing.status, activeLocale);
        ...
        <div className="cl-detail__body">
          ...
          {listing.condition && (
            <div className="cl-detail__condition">
              Condition:{" "}
              <strong>
                {listingOptionLabel(CONDITION_LABELS, listing.condition, activeLocale)}
              </strong>
            </div>
          )}

          {listing.description && <div className="cl-detail__description">{tr("description")}</div>}

          {listing.location && <div className="cl-detail__location">📍 {listing.location}</div>}
            ...
            <div className="cl-detail__actions">
              {listing.interested ? (
                <>
                  ...
                  <button className="button button-secondary" onClick={handleWithdraw}>
                    Withdraw
                  </button>
                </>
              ) : (
              ...
              <button
                ...
                className="button button-secondary cl-detail__watch-btn"
              >
          ...
          {canManage && (
            <div className="cl-detail__owner-actions">
              {isOpen && (
                <>
                  <Link
                    to={`/community_listings/${listing.id}/edit`}
                    className="button button-secondary"
                  >
                    Edit
                  </Link>
                  <button className="button button-secondary" onClick={handleFulfill}>
                    Mark as Fulfilled
                  </button>
              ...
              <button className="cl-detail__delete-btn" onClick={handleDelete}>
                Delete
              </button>
            </div>
          )}
