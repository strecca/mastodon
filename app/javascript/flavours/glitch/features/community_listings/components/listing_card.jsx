...
export const ListingCard = ({ listing }) => {
  ...
  const meta = TYPE_COLORS[listing.listing_type] ?? {
    bg: "#f5f5f5",
    color: "#555",
    ...
    <Link to={`/community_listings/${listing.id}`} className="cl-card">
      ...
      <div className="cl-card__body">
        <div className="cl-card__top">
          <span className="cl-card__type-badge" style={{ background: meta.bg, color: meta.color }}>
            {meta.label}
          </span>
        ...
        {desc && (
          <div className="cl-card__desc">{desc.length > 80 ? desc.slice(0, 80) + "…" : desc}</div>
        )}
        ...
        <div className="cl-card__footer">
          ...
          {listing.location && <span className="cl-card__location">· {listing.location}</span>}
          {listing.interest_count > 0 && (
            <span className="cl-card__interest-count">{listing.interest_count} interested</span>
          )}
        </div>
      </div>
    </Link>
