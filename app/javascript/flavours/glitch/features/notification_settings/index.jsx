...
const NotificationSettings = ({ multiColumn }) => {
  ...
  const toggleCategory = useCallback(
    (categoryKey) => {
      ...
      if (existing) {
        api()
          .delete(`/api/v1/member_notification_category_subscriptions/${existing.id}`)
          .then(() => setSubscriptions((prev) => prev.filter((s) => s.id !== existing.id)));
      } else {
        api()
          .post("/api/v1/member_notification_category_subscriptions", { category_key: categoryKey })
  ...
  const searchAccounts = useCallback((value) => {
    ...
    api()
      .get("/api/v1/accounts/search", { params: { q: value, limit: 5 } })
      .then((res) => setResults(res.data));
  }, []);

  const addTarget = useCallback((account) => {
    api()
      .post("/api/v1/member_notification_targets", { target_account_id: account.id })
      .then((res) => {
  ...
  const savePreferences = useCallback(() => {
    setSaving(true);
    api()
      .put("/api/v1/member_notification_preferences", preferences)
      .then((res) => {
  ...
  const callout = (
    <div className="ns-callout">
      ...
      <div className="ns-callout__ios">
        ...
        <Link to="/guide">
          {sc("notification_settings_callout_ios_link", "See the step-by-step guide")}
        </Link>
      </div>
  ...
  return (
    <Column>
      ...
      <Helmet>
        <title>Notification Settings · miacivezza</title>
      </Helmet>
      <div className="ns-page">
        {callout}
        ...
        <section className="ns-section">
          ...
          <p className="ns-section__hint">
            Choose which Community Directory categories notify you when someone posts something new.
          </p>
        ...
        <section className="ns-section">
          ...
          <p className="ns-section__hint">
            Specific members whose posts you want to hear about, regardless of category.
          </p>
          <div className="ns-targets">
            {targets.map((t) => (
              <span key={t.id} className="ns-targets__chip">
                @{t.account.username}
                <button type="button" onClick={() => removeTarget(t.id)} aria-label="Remove">
                  ×
                </button>
              </span>
            ))}
        ...
        <section className="ns-section">
          <h3>Quiet hours</h3>
          <p className="ns-section__hint">
            Push notifications will hold off during this window and arrive once it ends.
          </p>
