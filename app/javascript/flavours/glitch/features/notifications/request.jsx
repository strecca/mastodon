...
export const NotificationRequest = ({ multiColumn, params: { id } }) => {
  ...
  const notificationRequest = useSelector((state) =>
    state.notificationRequests.current.item?.id === id
      ? state.notificationRequests.current.item
      : null,
  );
  const accountId = notificationRequest?.account_id;
  const account = useSelector((state) => state.getIn(["accounts", accountId]));
  const notifications = useSelector(
    (state) => state.notificationRequests.current.notifications.items,
  );
  const isLoading = useSelector(
    (state) => state.notificationRequests.current.notifications.isLoading,
  );
  ...
  const columnTitle = intl.formatMessage(messages.title, {
