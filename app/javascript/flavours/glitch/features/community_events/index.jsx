...
const messages = defineMessages({
  cta: {
    id: "community.events.cta",
    defaultMessage: "Log In or Join to add your own Community Events",
  },
  postEvent: { id: "community.events.post_event", defaultMessage: "Post an Event" },
  searchPlaceholder: {
    id: "community.events.search_placeholder",
    defaultMessage: "Search by title, category or location…",
  },
  ...
  headerUpcomingToday: {
    id: "community.events.header.upcoming_today",
    defaultMessage: "Upcoming Events: Today",
  },
  headerUpcomingThisWeek: {
    id: "community.events.header.upcoming_this_week",
    defaultMessage: "Upcoming Events: This Week",
  },
  headerUpcomingFilter: {
    id: "community.events.header.upcoming_filter",
    defaultMessage: "Upcoming Events: {filter}",
  },
  ...
  eventCount: {
    id: "community.events.event_count",
    defaultMessage: "{count, plural, one {# event} other {# events}}",
  },
  myEvents: { id: "community.events.my_events", defaultMessage: "My Events" },
  myEventsEmpty: {
    id: "community.events.my_events_empty",
    defaultMessage: "You haven't posted any events yet. Share what's happening in the community!",
  },
  whoInTown: { id: "community.events.who_in_town", defaultMessage: "Who’s in town?" },
  whoInTownDesc: {
    id: "community.events.who_in_town_desc",
    defaultMessage: "See which members will be visiting around event dates",
  },
  ...
  autoImportedFrom: {
    id: "community.events.auto_imported_from",
    defaultMessage: "Auto-imported from {source}",
  },
  ...
  ariaDayEvents: {
    id: "community.events.aria.day_events",
    defaultMessage: "{day} — {count, plural, one {# event} other {# events}}",
  },
  ariaSwitchToCalendar: {
    id: "community.events.aria.switch_to_calendar",
...
const EventDayPanel = ({ date, events, onClose, accountId }) => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={intl.formatMessage(messages.ariaEventsOn, {
        date: fmtDateShort(date, intl.locale),
      })}
    >
      <div className="cv-modal ce-day-panel">
        <div className="cv-day-panel__header">
          ...
          <button
            ...
            onClick={onClose}
            aria-label={intl.formatMessage(messages.ariaClose)}
          >
            <CloseIcon />
          </button>
        </div>
            ...
            return (
              <li key={evt.id} className="ce-day-panel__event">
                <div className="ce-day-panel__event-header">
                  ...
                  {isOwn && (
                    <Link
                      to={`/community_events/${evt.id}/edit`}
                      className="cv-icon-btn"
                      title={intl.formatMessage(messages.ariaEditEvent)}
                    >
                      <EditIcon />
                    </Link>
                  )}
                ...
                {evt.category?.length > 0 && (
                  <div className="cv-day-panel__chips">
                    {evt.category.map((c) => (
                      <span key={c} className="cv-day-panel__chip ce-day-panel__chip">
                        {c}
                      </span>
                    ))}
                  </div>
    ...
    <div className={`ce-list__row${isExpanded ? " ce-list__row--expanded" : ""}`}>
      ...
      >
        ...
        {event.category?.length > 0 && (
          <span className="ce-list__cat-preview">{event.category.slice(0, 2).join(" · ")}</span>
        )}
        {event.auto_imported && event.source_name && (
          <span
            className="ce-list__source-badge"
            title={intl.formatMessage(messages.autoImportedFrom, { source: event.source_name })}
          >
            {event.source_name}
          </span>
        )}
        ...
        <div className="ce-list__detail">
          <div className="ce-list__detail-inner">
              ...
              {event.category?.length > 0 && (
                <div className="ce-list__detail-chips">
                  {event.category.map((c) => (
                    <span key={c} className="ce-list__chip">
                      {c}
                    </span>
                  ))}
                </div>
              ...
              <div className="ce-list__detail-source">
                ...
                {event.source_url ? (
                  <a
                    ...
                    className="ce-list__source-link"
                  >
                    {event.source_name}
                  </a>
                ) : (
            ...
            <div className="ce-list__detail-actions">
              <Link
                to={`/community_events/${event.id}`}
                className="button button--compact ce-list__action-btn ce-list__action-btn--primary"
              >
                {intl.formatMessage(messages.viewDetails)}
              </Link>
              {isOwn && (
                <Link
                  to={`/community_events/${event.id}/edit`}
                  className="button button--compact ce-list__action-btn ce-list__action-btn--edit"
                >
...
const EventDateGroup = ({ dateKey, events, expandedId, onToggle, accountId }) => {
  ...
  return (
    <div className="ce-list__group">
      <div
        className={`ce-list__date-heading${isToday ? " ce-list__date-heading--today" : isTomorrow ? " ce-list__date-heading--tomorrow" : ""}`}
      >
        {headingText}
    ...
    <div className={`ce-my-card${isPast ? " ce-my-card--past" : ""}`}>
      ...
      {event.category?.length > 0 && (
        <div className="ce-my-card__chips">
          {event.category.map((c) => (
            <span key={c} className="ce-my-card__chip">
              {c}
            </span>
          ))}
        </div>
      ...
      <div className="ce-my-card__actions">
        <Link
          to={`/community_events/${event.id}/edit`}
          className="cv-icon-btn"
          title={intl.formatMessage(messages.ariaEdit)}
        >
          <EditIcon />
        </Link>
      </div>
...
const CommunityEvents = ({ multiColumn }) => {
  ...
  const listEvents = useMemo(() => {
    ...
    return [...base].sort((a, b) => {
      ...
      const pa = isPriorityEvent(a),
        pb = isPriorityEvent(b);
  ...
  const listHeader = useMemo(() => {
    ...
    const t = new Date();
    const weEnd = new Date(t);
    weEnd.setDate(t.getDate() + (6 - t.getDay()));
      ...
      <div className="ce-page">
        ...
        <div className="cv-filters ce-filters">
          ...
          {hasActiveFilters && (
            <button
              type="button"
              className="cv-filter-chip ce-filter-chip ce-filter-chip--clear"
        ...
        {view === "calendar" && (
          <>
            <div className="ce-cal__nav">
              <button
                className="ce-nav-btn"
                onClick={prevMonth}
                aria-label={intl.formatMessage(messages.ariaPrevMonth)}
              >
                <ChevronLeftIcon />
              </button>
            ...
            <div className="ce-cal__view-toggle-row">
              <button type="button" className="ce-view-toggle" onClick={() => setView("list")}>
            ...
            <div className="cv-cal ce-cal">
              {weekdayLabels.map((d) => (
                <div key={d} className="cv-cal__weekday ce-cal__weekday">
                  {d}
                </div>
              ))}
                    ...
                    return (
                      <div
                        ...
                        onKeyDown={
                          clickable
                            ? (e) =>
                                (e.key === "Enter" || e.key === " ") && openDayPanel(key, dayEvents)
                            : undefined
                        }
                        aria-label={
                          clickable
                            ? intl.formatMessage(messages.ariaDayEvents, {
                                day: day.getDate(),
        ...
        {signedIn && (
          <div className="cv-my-section ce-my-section">
            <div className="cv-my-section__header">
              <h3 className="cv-my-section__title ce-my-section__title">
            ...
            {myEvents.length === 0 ? (
              <p className="cv-my-section__empty">{intl.formatMessage(messages.myEventsEmpty)}</p>
            ) : (
