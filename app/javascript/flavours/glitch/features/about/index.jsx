...
const AboutBio = () => {
  ...
  return (
    <div className="about__header__bio">
      {photoUrl && <img src={photoUrl} alt="About the site" className="about__header__bio-photo" />}
      <p>
        {sc(
          "about_bio_p1",
              ...
              <Link to="/landing">
                <ServerHeroImage
                  ...
                  srcSet={Object.keys(server.item?.thumbnail.versions ?? {})
                    .map(
                      (key) =>
                        `${server.item?.thumbnail.versions && server.item.thumbnail.versions[key]} ${key.replace("@", "")}`,
                    )
                    .join(", ")}
                    ...
                    {domainBlocks.items.map((block) => (
                      <div className="about__domain-blocks__domain" key={block.domain}>
                        <div className="about__domain-blocks__domain__header">
                          <h6>
                            <span title={`SHA-256: ${block.digest}`}>{block.domain}</span>
                          </h6>
                          <span
                            className="about__domain-blocks__domain__type"
                            title={intl.formatMessage(severityMessages[block.severity].explanation)}
                          >
