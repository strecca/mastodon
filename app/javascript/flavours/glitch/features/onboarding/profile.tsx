...
import { TextAreaField, TextInputField, Toggle } from "flavours/glitch/components/form_fields";
...
const nullIfMissing = (path: string) => (path.endsWith("missing.png") ? null : path);
...
export const Profile: React.FC<{
  ...
  const [discoverable, setDiscoverable] = useState(account?.discoverable ?? true);
  ...
  const avatarPreview = useMemo(
    () => (avatar ? URL.createObjectURL(avatar) : nullIfMissing(account?.avatar ?? "missing.png")),
    [avatar, account],
  );
  const headerPreview = useMemo(
    () => (header ? URL.createObjectURL(header) : nullIfMissing(account?.header ?? "missing.png")),
    [header, account],
  );
      ...
      <div className="scrollable scrollable--flex">
        <div className="simple_form app-form">
          ...
          <div className="fields-group">
            <TextAreaField
              maxLength={500}
              label={<FormattedMessage id="onboarding.profile.note" defaultMessage="Bio" />}
              hint={
                <FormattedMessage
        ...
        <div className="column-footer">
          <Button block onClick={handleSubmit} disabled={isSaving}>
            {isSaving ? (
              ...
              <FormattedMessage id="onboarding.profile.finish" defaultMessage="Finish" />
            )}
          </Button>
        </div>
      </div>
