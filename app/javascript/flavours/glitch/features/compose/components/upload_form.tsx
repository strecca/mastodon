...
import type { List, Map as ImmutableMap, List as ImmutableList } from "immutable";
...
const messages = defineMessages({
  ...
  onDragCancel: {
    ...
    defaultMessage: "Dragging was cancelled. Media attachment {item} was dropped.",
  },
});
...
export const UploadForm: React.FC = () => {
  ...
  const active = useAppSelector((state) => state.compose.get("is_uploading") as boolean);
  const progress = useAppSelector((state) => state.compose.get("progress") as number);
  const isProcessing = useAppSelector((state) => state.compose.get("is_processing") as boolean);
  ...
  return (
    <>
      ...
      {mediaIds.size > 0 && (
        ...
        >
          {mediaIds.size === 1 ? (
            <Upload id={mediaIds.first()} dragging={false} draggable={false} tall wide />
          ) : (
            <DndContext
              sensors={sensors}
