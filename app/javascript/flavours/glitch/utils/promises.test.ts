...
describe("onceAsyncByArgs", () => {
  ...
  test("calls log function when provided", async () => {
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("added promise with key"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("already have in-flight promise with key"),
    );
    expect(log).toHaveBeenCalledWith(expect.stringContaining("deleted completed promise with key"));
  });
