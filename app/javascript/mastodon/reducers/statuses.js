...
const statusTranslateUndo = (state, id) => {
  return state.withMutations((map) => {
    ...
    map
      ...
      .forEach((item, index) => map.deleteIn([id, "media_attachments", index, "translation"]));
  });
};
  ...
  switch (action.type) {
    ...
    case STATUS_FETCH_FAIL: {
      if (action.parentQuotePostId && action.error.status === 404) {
        return removeStatusStub(state, action.id).setIn(
          [action.parentQuotePostId, "quote", "state"],
          "deleted",
        );
      } else {
    ...
    default:
      if (reblog.pending.match(action))
        ...
        return state.get(action.meta.arg.statusId) === undefined
          ? state
          : state.setIn([action.meta.arg.statusId, "reblogged"], false);
        ...
        return state.get(action.meta.arg.statusId) === undefined
          ? state
          : state.setIn([action.meta.arg.statusId, "reblogged"], true);
      else return state;
  }
}
