...
export const StatusThreadLabel: React.FC<{
  ...
  const inReplyToAccount = useAppSelector((state) => state.accounts.get(inReplyToAccountId));
  ...
  if (accountId === inReplyToAccountId) {
    ...
    label = (
      <FormattedMessage
        ...
        values={{
          name: (
            <LinkedDisplayName displayProps={{ account: inReplyToAccount, variant: "simple" }} />
          ),
        }}
      />
    );
