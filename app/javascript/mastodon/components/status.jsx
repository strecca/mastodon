...
class Status extends ImmutablePureComponent {
  ...
  getAttachmentAspectRatio() {
    ...
    if (attachments.getIn([0, "type"]) === "video") {
      ...
      return attachments.size === 1 && attachments.getIn([0, "meta", "small", "aspect"])
  ...
  handleHotkeyOpenMedia = (e) => {
    ...
    if (status.get("media_attachments").size > 0) {
      ...
      if (status.getIn(["media_attachments", 0, "type"]) === "video") {
        onOpenVideo(status.get("id"), status.getIn(["media_attachments", 0]), lang, {
          startTime: 0,
        });
      } else {
    ...
    if (status.get("reblog", null) !== null && typeof status.get("reblog") === "object") {
      ...
      prepend = (
        <StatusThreadLabel
          ...
          inReplyToAccountId={status.get("in_reply_to_account_id")}
        />
      );
    }
    ...
    return (
      <Hotkeys handlers={handlers} focusable={!unfocusable}>
        <div
          ...
          tabIndex={this.props.muted || unfocusable ? null : 0}
          data-featured={featured ? "true" : null}
          aria-label={textForScreenReader({ intl, status, rebloggedByText, isQuote: isQuotedPost })}
          ref={this.handleRef}
