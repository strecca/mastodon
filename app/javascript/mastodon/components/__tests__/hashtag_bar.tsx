...
function createStatus(content: string, hashtags: string[], hasMedia = false, spoilerText?: string) {
  return fromJS({
...
describe("computeHashtagBarForStatus", () => {
  it("does nothing when there are no tags", () => {
    ...
    const { hashtagsInBar, statusContentProps } = computeHashtagBarForStatus(status);

    expect(hashtagsInBar).toEqual([]);
    expect(statusContentProps.statusContent).toMatchInlineSnapshot(`"<p>Simple text</p>"`);
  });

  it("displays out of band hashtags in the bar", () => {
    const status = createStatus('<p>Simple text <a href="test">#hashtag</a></p>', [
      "hashtag",
      "test",
    ]);
  ...
  it("extract tags from the last line", () => {
    const status = createStatus('<p>Simple text</p><p><a href="test">#hashtag</a></p>', [
      "hashtag",
    ]);
  ...
  it("de-duplicate accentuated characters with case differences", () => {
    ...
    expect(statusContentProps.statusContent).toMatchInlineSnapshot(`"<p>Text</p>"`);
  });
  ...
  it("does not modify a status with a line of hashtags only", () => {
    const status = createStatus('<p><a href="test">#test</a>  <a href="test">#hashtag</a></p>', [
      "test",
      "hashtag",
    ]);
