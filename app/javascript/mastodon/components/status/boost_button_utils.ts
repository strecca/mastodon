...
export const selectStatusState = createAppSelector(
  [(state) => state.meta.get("me") as string | undefined, (_, status: Status) => status],
  (userId, status) => {
    const isPublic = ["public", "unlisted"].includes(status.get("visibility") as StatusVisibility);
    const isMineAndPrivate =
      userId === status.getIn(["account", "id"]) && status.get("visibility") === "private";
    return {
      isLoggedIn: !!userId,
      isPublic,
...
export function quoteItemState({
  ...
  if (!isPublic && !isMine) {
    ...
    iconText.disabled = true;
    iconText.iconComponent = FormatQuoteOff;
    iconText.meta = isQuoteFollowersOnly ? messages.quote_followers_only : messages.quote_cannot;
  }
