    ...
    <div className={classNames("story", { expanded })}>
      <div className="story__details">
        <div className="story__details__publisher">
          {publisher ? <span lang={lang}>{publisher}</span> : <Skeleton width={50} />}
          {publishedAt && (
            <>
              {" "}
        ...
        <div className="story__details__shared">
          {author ? (
            <FormattedMessage
              ...
              className="story__details__shared__author"
              defaultMessage="By {name}"
              values={{
                name: authorAccount ? (
                  <AuthorLink accountId={authorAccount} />
                ) : (
          ...
          {typeof sharedTimes === "number" ? (
            <Link className="story__details__shared__pill" to={`/links/${encodeURIComponent(url)}`}>
              <ShortNumber value={sharedTimes} renderer={sharesCountRenderer} />
            </Link>
          ) : (
            <Skeleton width="10ch" />
          )}
