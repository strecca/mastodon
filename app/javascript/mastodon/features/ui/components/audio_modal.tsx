...
const AudioModal: React.FC<{
  ...
  useEffect(() => {
    const backgroundColor = getAverageFromBlurhash(media.get("blurhash") as string | null);
    ...
    <div className="modal-root__modal audio-modal">
      <div className="audio-modal__container">
        <Audio
          ...
          alt={description}
          lang={language}
          poster={(media.get("preview_url") as string | null) ?? accountStaticAvatar}
      ...
      <div className="media-modal__overlay">
        {status && (
          <Footer statusId={status.get("id") as string} withOpenButton onClose={onClose} />
        )}
      </div>
    </div>
  );
