...
export const BundleColumnError: React.FC<BundleColumnErrorProps> = ({
  ...
  switch (errorType) {
    ...
    case "network":
      title = (
        <FormattedMessage id="bundle_column_error.network.title" defaultMessage="Network error" />
      );
      body = (
        <FormattedMessage
  ...
  return (
    <Column bindToDocument={!multiColumn}>
      <EmptyState image="error" title={title} message={body} className={classes.error}>
        <div className={classes.actions}>
          {errorType === "network" && onRetry && (
            <Button onClick={onRetry}>
              <FormattedMessage id="bundle_column_error.retry" defaultMessage="Try again" />
            </Button>
          )}
          ...
          >
            <FormattedMessage id="bundle_column_error.return" defaultMessage="Go back home" />
          </Link>
        </div>
      </EmptyState>
