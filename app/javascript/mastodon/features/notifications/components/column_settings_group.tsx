...
const selectNotificationSettings = createAppSelector(
  [
    (state) =>
      (state.settings as Immutable.Map<string, unknown>).get("notifications") as Immutable.Map<
        string,
  ...
  (settings, browserPermission, pushSettings) => ({
    ...
    pushSettings,
    showPushSettings: pushSettings.get("supported") && pushSettings.get("enabled"),
  }),
);
...
export const ColumnSettingsGroup: FC<{ label: ReactNode; type: string }> = ({ label, type }) => {
  const { settings, browserPermission, pushSettings, showPushSettings } = useAppSelector(
