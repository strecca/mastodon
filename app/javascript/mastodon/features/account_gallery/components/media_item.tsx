...
export const MediaItem: React.FC<{
  ...
  const [visible, setVisible] = useState(
    (displayMedia !== "hide_all" && !attachment.getIn(["status", "sensitive"])) ||
  ...
  if (!visible) {
    ...
    const duration = attachment.getIn(["meta", "original", "duration"]) as number;
  ...
  return (
    ...
    >
      ...
      {badges.length > 0 && <div className="media-gallery__item__badges">{badges}</div>}
    </div>
  );
};
