...
export const NotificationGroupWithStatus: React.FC<{
  ...
  return (
    <Hotkeys handlers={handlers}>
      <div
        role="button"
        className={classNames(`notification-group focusable notification-group--${type}`, {
      ...
      >
        ...
        <div className="notification-group__main">
          <div className="notification-group__main__header">
            <div className="notification-group__main__header__wrapper">
              <AvatarGroup avatarHeight={AVATAR_SIZE}>
                {accountIds.slice(0, NOTIFICATIONS_GROUP_MAX_AVATARS).map((id) => (
                  <AvatarById key={id} accountId={id} />
                ))}
              </AvatarGroup>

              {actions && <div className="notification-group__actions">{actions}</div>}
            </div>
          ...
          {additionalContent && (
            <div className="notification-group__main__additional-content">{additionalContent}</div>
          )}
        </div>
      </div>
    </Hotkeys>
