  ...
  return (
    <FormattedMessage
      ...
      values={{
        name: displayedName,
        count: total - 1,
        a: (chunks) => (seeMoreHref ? <Link to={seeMoreHref}>{chunks}</Link> : chunks),
      }}
    />
  );
};

const privateLabelRenderer: LabelRenderer = (displayedName, total, seeMoreHref) => {
  if (total === 1)
    return (
      <FormattedMessage
...
export const NotificationFavourite: React.FC<{
  ...
  const statusAccount = useAppSelector(
    (state) => state.accounts.get(state.statuses.getIn([statusId, "account"]) as string)?.acct,
  );
  ...
  return (
    <NotificationGroupWithStatus
      ...
      labelSeeMoreHref={statusAccount ? `/@${statusAccount}/${statusId}/favourites` : undefined}
      unread={unread}
    />
  );
};
