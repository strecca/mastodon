...
describe("tokenizeText", () => {
  ...
  test("does not capture custom emoji with invalid characters", () => {
    expect(tokenizeText("Hello :smile-123:!!")).toEqual(["Hello :smile-123:!!"]);
  });
});
...
describe("updateHtmlWithEmoji", () => {
  ...
  test("updates element text with emojis", async () => {
    ...
    vi.spyOn(db, "loadLegacyShortcodesByShortcode").mockResolvedValueOnce(undefined);
    vi.spyOn(db, "loadEmojiByHexcode").mockResolvedValueOnce(unicodeEmojiFactory());
...
describe("loadEmojiDataToState", () => {
  ...
  test("loads unicode data into state", async () => {
    const dbCall = vi.spyOn(db, "loadEmojiByHexcode").mockResolvedValue(unicodeEmojiFactory());
    const dbLegacyCall = vi.spyOn(db, "loadLegacyShortcodesByShortcode").mockResolvedValueOnce({
  ...
  test("retries loading emoji data once if initial load fails", async () => {
    ...
    const consoleCall = vi.spyOn(console, "warn").mockImplementationOnce(() => null);
