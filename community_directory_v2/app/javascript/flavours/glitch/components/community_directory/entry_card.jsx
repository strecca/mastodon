...
export const EntryCard = ({ entry, config, featureKey }) => {
  ...
  const nameField = config.fields.find(
    (f) => f.db_name === "display_name" || f.db_name === "name" || f.db_name === "title",
  );
  ...
  const previewFields = config.fields.filter((f) => f.db_name !== nameField?.db_name).slice(0, 3);
  ...
  return (
    <div className="community-entry-card">
      ...
      <Link to={`/${featureKey}/${id}`} className="community-entry-card__title">
          ...
          {previewFields.map((field) => {
            ...
            return (
              <div key={field.db_name} className="community-entry-card__field">
                ...
                <span className="community-entry-card__field-value">{truncated}</span>
              </div>
            );
          })}
        </div>
