...
const startServer = async () => {
  ...
  const checkScopes = (req, logger, channelName) =>
    new Promise((resolve, reject) => {
      ...
      if (
        req.scopes &&
        requiredScopes.some((requiredScope) => req.scopes.includes(requiredScope))
      ) {
        resolve();
        return;
      }
  ...
  const getFeedAccessSettings = async (kind, req) => {
    ...
    const result = await pgPool.query("SELECT var, value FROM settings WHERE var IN ($1, $2)", [
      localAccessVar,
      remoteAccessVar,
    ]);
  ...
  const streamFrom = (
    ...
    attachCloseHandler,
    destinationType,
    { needsFiltering, filterLocal, filterRemote, allowLocalOnly } = {
  ...
  const channelNameToIds = (req, name, params) =>
    new Promise((resolve, reject) => {
      ...
      const resolveFeed = (feedKind, channelId, options) => {
        getFeedAccessSettings(feedKind, req)
          .then(({ localAccess, remoteAccess }) => {
            resolve({
      ...
      switch (name) {
        ...
        case "user:notification":
          ...
          break;
        case "public":
          resolveFeed("public", "timeline:public", {
            needsFiltering: true,
        ...
        case "public:allow_local_only":
          ...
          break;
        case "public:local":
          resolveFeed("public", "timeline:public:local", {
            needsFiltering: true,
            allowLocalOnly: true,
          });
          break;
        case "public:remote":
          resolveFeed("public", "timeline:public:remote", {
            needsFiltering: true,
            allowLocalOnly: false,
          });
          break;
        case "public:media":
          resolveFeed("public", "timeline:public:media", {
            needsFiltering: true,
            allowLocalOnly: isTruthy(params.allow_local_only),
          });
          break;
        ...
        case "public:local:media":
          resolveFeed("public", "timeline:public:local:media", {
            needsFiltering: true,
            allowLocalOnly: true,
          });
          break;
        case "public:remote:media":
          resolveFeed("public", "timeline:public:remote:media", {
            needsFiltering: true,
            allowLocalOnly: false,
          });
          break;
        ...
        case "hashtag":
          ...
          resolveFeed("hashtag", `timeline:hashtag:${normalizeHashtag(params.tag)}`, {
            needsFiltering: true,
            allowLocalOnly: true,
          });
        ...
        case "hashtag:local":
          ...
          resolveFeed("hashtag", `timeline:hashtag:${normalizeHashtag(params.tag)}:local`, {
            needsFiltering: true,
            allowLocalOnly: true,
          });
